...
class NeuroGymTask(Task):
    ...
    @classmethod
    def _get_neurogym(cls):
        """Lazily import neurogym only when NeuroGymTask is actually used."""
        if cls._ngym is None:
            try:
                import neurogym as ngym
            except ImportError as exc:
                raise ImportError(
                    "NeuroGymTask requires the optional dependency 'neurogym'. "
                    "Install it with: uv sync --extra neurogym "
                    'or pip install "conn2res[neurogym]".'
                ) from exc
            cls._ngym = ngym
        return cls._ngym
    ...
    def fetch_data(self, n_trials=None, input_gain=None, add_bias=False, **kwargs):
        """
        ...
        ngym = self._get_neurogym()

        # create a Dataset object from NeuroGym
        dataset = ngym.Dataset(self._name + "-v0", env_kwargs=kwargs)
...
class ReservoirPyTask(Task):
    ...
    def fetch_data(
        ...
        **kwargs,
        ...
        n_total = self.n_trials + win + abs_horizon_max + 1
        result = env(n_timesteps=n_total, **kwargs)

        # reservoirpy>=0.4 may return (input, target) tuple for some tasks;
        # take only the input and trim to expected length
...
class Conn2ResTask(Task):
    ...
    def fetch_data(
        ...
        # generate input data
        x = rng.uniform(
            low=low, high=high, size=(self.n_trials + win + abs_horizon_max + 1)
        )[:, np.newaxis]
