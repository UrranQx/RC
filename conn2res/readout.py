    ...
    def test(self, X, y, sample_weight=None, metric=None, **kwargs):
        ...
        for m in metric:
            if m == "score":
                ...
                scores[m] = self._model.score(X, y, sample_weight=sample_weight)
            else:
                # get score function
                ...
                # estimate score
                scores[m] = func(y, y_pred, sample_weight=sample_weight, **kwargs)
    ...
    def run_task(
        ...
        **kwargs,
        ...
        if readout_nodes is None:
            self.train(x_train, y_train, sample_weight_train)
            ...
            if all(isinstance(i, (list, tuple, np.ndarray)) for i in readout_nodes):
                ...
                for i, readouts in zip(ids, readout_nodes):
                    self.train(x_train[:, readouts], y_train, sample_weight_train)
            ...
            # readout_nodes is a single array
                ...
                self.train(x_train[:, readout_nodes], y_train, sample_weight_train)

                score = self.test(
                    x_test[:, readout_nodes],
                    y_test,
                    sample_weight_test,
                    metric=metric,
                    **kwargs,
...
def select_model(y):
    ...
    if y.dtype in [np.int32, np.int64]:
        if y.squeeze().ndim == 1:
            ...
            )  # 'multioutput_classification' + 'multioutput_multiclass_classification'
        else:
        ...
        if y.squeeze().ndim == 1:
            ...
            # double check that values are actually continuos
            is_integer = [all(isinteger(y[:, col])) for col in range(y.shape[1])]
            if all(is_integer):
...
def _sample_weight(y, split_set, seed=None):
    ...
    if split_set == "train":
        ...
        sample_weight[idx] = rng.rand(
            (len(idx))
        )  # rng.rand vs rng.random: We need to deicde if its a bug and fix it then.
...
def _get_sample_weight_old(inputs, labels=None, sample_block=None):
    ...
    if sample_block is None:
        for data in inputs:
            ...
            icol = [
                col for col in range(data.shape[1]) if np.unique(data[:, col]).size <= 3
            ]  # class is based on <=3 real values

            _, sample_block = np.unique(data[:, icol], return_inverse=True, axis=0)

            # get unique sample blocks
            _, ia, nc = np.unique(sample_block, return_index=True, return_counts=True)
