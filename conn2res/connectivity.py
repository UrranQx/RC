...
import numpy as np
from bct import backbone_wu, distance_bin, get_components, reference
...
DATA_DIR = os.environ.get("CONN2RES_DATA_DIR", os.path.join(PROJ_DIR, "data", "human"))
...
class Conn:
    ...
    def __init__(self, w=None, filename=None, subj_id=0, modules=None, density=None):
        """
    ...
    def randomize(self, swaps=10, **kwargs):
        ...
        if utils.check_symmetric(self.w):
            self.w, _ = reference.randmio_und_connected(self.w, swaps, **kwargs)
        else:
            self.w, _ = reference.randmio_dir_connected(self.w, swaps, **kwargs)
    ...
    def threshold(self, threshold=1, **kwargs):
        ...
        if utils.check_symmetric(self.w):
            _, self.w = backbone_wu(self.w, (self.n_nodes - 1) * threshold, **kwargs)
        else:
            raise ValueError("threshold is only implemented for symmetric networks")

        # update class attributes
        self._update_attributes(np.isin(np.arange(self.n_nodes), np.nonzero(self.w)[0]))
    ...
    def add_weights(self, w, mask="triu", order="random", seed=None):
        ...
        if mask == "full":
            if w.size != self.n_edges:
                raise ValueError("number of elements in mask and w do not match")
            ...
            if not self.symmetric:
                raise ValueError(
                    ...
                    ", order="
                    "random"
                    ") needs a symmetric connectivity matrix"
                )
    ...
    def subset_nodes(self, node_set="all", idx_node=None, **kwargs):
        ...
        if idx_node is None:
            idx_node = np.isin(
                np.arange(self.n_nodes), self.get_nodes(node_set, **kwargs)
            )
        ...
        if self.symmetric:
            ...
            warnings.warn(
                "Asymmetric connectivity matrix is only weakly checked for connectedness."
            )
    ...
    def get_nodes(
        ...
        **kwargs,
        ...
        filename : str, optional
            ...
            path (by default, the data is loaded from data/human), which
        ...
        if node_set == "all":
            ...
            # select random nodes
            selected_nodes = rng.choice(nodes_from, size=n_nodes, replace=False)
            ...
            if len(np.unique(node_pairs)) >= n_nodes:
                ...
                raise ValueError("n_nodes do not exist with given shortest_path")
            ...
            if node_set in module_ids:
                ...
                selected_nodes = [
                    e for i, e in enumerate(modules) if (module_ids == node_set)[i]
                ][0]
