...
from conn2res.connectivity import Conn, get_modules, load_file
...
def create_random_connectivity(zero_ratio=0.5):
    ...
    indices_x = np.random.choice(
        np.arange(U.shape[0]),
        replace=True,
        size=int(U.shape[0] * U.shape[1] * zero_ratio),
    )
...
class TestConn:
    def test_scale_and_normalize(self):
        ...
        assert nonzero_count_U == nonzero_count_asym, (
            "normalization changed number of directed edges"
        )
        assert nonzero_count_S == nonzero_count_sym, (
            "normalization changed number of undirected edges"
        )
        assert ((0 <= test_conn_asym.w) & (test_conn_asym.w <= 1)).all(), (
            "asymmetric connectivity not normalized"
        )
        assert ((0 <= test_conn_sym.w) & (test_conn_sym.w <= 1)).all(), (
            "symmetric connectivity not normalized"
        )
        assert not issymmetric(test_conn_asym.w), (
            "asymmetric no longer asymmetric after normalization"
        )
        assert issymmetric(test_conn_sym.w), (
            "symmetric no longer symmetric after normalization"
        )
    ...
    def test_binarize(self):
        ...
        assert ((0 == test_conn_asym.w) | (test_conn_asym.w == 1)).all(), (
            "asymmetric connectivity not binarized"
        )
        assert ((0 == test_conn_sym.w) | (test_conn_sym.w == 1)).all(), (
            "symmetric connectivity not binarized"
        )
        assert not issymmetric(test_conn_asym.w), (
            "asymmetric no longer asymmetric after binarization"
        )
        assert issymmetric(test_conn_sym.w), (
            "symmetric no longer symmetric after binarization"
        )
    ...
    def test_randomize(self):
        ...
        assert not nx.is_isomorphic(orig_conn_asym_G, conn_asym), (
            "asymmetric connectivity not randomized"
        )
        assert not nx.is_isomorphic(orig_conn_sym_G, conn_sym), (
            "symmetric connectivity not randomized"
        )
        ...
        assert not issymmetric(test_conn_asym.w), (
            "asymmetric no longer asymmetric after randomization"
        )
        assert issymmetric(test_conn_sym.w), (
            "symmetric no longer symmetric after randomization"
        )
    ...
    def test_subset_nodes(self):
        ...
        max_conn_asym = test_conn_asym_G.subgraph(
            max(nx.connected_components(test_conn_asym_G), key=len)
        ).copy()
        max_conn_sym = test_conn_sym_G.subgraph(
            max(nx.connected_components(test_conn_sym_G), key=len)
        ).copy()
        ...
        assert nx.is_isomorphic(max_conn_asym, test_conn_asym), (
            "asymmetric connectivity induced subgraph not isomorphic to largest connected component"
        )
        assert nx.is_isomorphic(max_conn_sym, test_conn_sym), (
            "symmetric connectivity induced subgraph not isomorphic to largest connected component"
        )
    ...
    def test_get_nodes(self):
        ...
        for partition in partition_set:
            node_selection = test_conn_asym.get_nodes(
                partition, filename="test_partition.npy"
            )
            assert np.isin(node_selection, partition_dict[partition]).all(), (
                "module mismatch when getting node indices from specified module"
            )
            # Test single options
            ...
            assert np.isin(target_nodes, partition_dict[partition]).all(), (
                "module mismatch when getting all nodes"
            )
            target_nodes = test_conn_asym.get_nodes(
                "random", nodes_from=node_selection, n_nodes=5
            )
            assert np.isin(target_nodes, partition_dict[partition]).all(), (
                "module mismatch when getting random nodes"
            )
        ...
        for partition in partition_set:
            ...
            assert not np.isin(target_nodes, partition_dict[partition]).any(), (
                "node from excluded modules when getting all nodes"
            )
            target_nodes = test_conn_asym.get_nodes(
                "random", nodes_without=node_selection, n_nodes=5
            )
            assert not np.isin(target_nodes, partition_dict[partition]).any(), (
                "node from excluded modules when getting random nodes"
            )
        ...
        for set_size in range(2, len(partition_set)):
            ...
            for set in set_list:
                ...
                for subnet in partition:
                    total_partition_indices = np.concatenate(
                        (total_partition_indices, partition_dict[subnet])
                    )
                node_selection = test_conn_asym.get_nodes(
                    partition.tolist(), filename="test_partition.npy"
                )
                assert np.isin(node_selection, total_partition_indices).all(), (
                    "module mismatch when getting node indices from multiple modules"
                )
                # Test single options
                target_nodes = test_conn_asym.get_nodes(
                    "all", nodes_from=node_selection
                )
                assert np.isin(target_nodes, total_partition_indices).all(), (
                    "multiple modules mismatch when getting all nodes"
                )
                target_nodes = test_conn_asym.get_nodes(
                    "random", nodes_from=node_selection, n_nodes=5
                )
                assert np.isin(target_nodes, total_partition_indices).all(), (
                    "multiple modules mismatch when getting random nodes"
                )
                # target_nodes = test_conn_asym.get_nodes('shortest_path', nodes_from=node_selection, n_nodes=2)
                # assert np.isin(target_nodes, total_partition_indices).all()
...
def test_get_modules():
    ...
    unique_chars = set(
        np.random.choice([chr(i) for i in range(65, 91)], size=26, replace=False)
    )
    ...
    mean = (
        len(unique_chars) / 2
    )  # Adjust mean to center around the middle of the alphabet
    ...
    index_mapping = np.clip(np.round(normal_values), 0, len(unique_chars) - 1).astype(
        int
    )
    ...
    random_array = np.array(list(unique_chars))[
        np.random.choice(index_mapping, size=array_length, replace=True)
    ]
    ...
    assert np.array_equal(module_ids_asym, unique_chars), (
        "module array mismatched for graph"
    )
    assert np.array_equal(readout_modules_asym, random_array_indices), (
        "module array indexing mismatched for graph"
    )
