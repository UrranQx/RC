...
class EchoStateNetwork(Reservoir):
    ...
    def simulate(
        self, ext_input, w_in, ic=None, output_nodes=None, return_states=True, **kwargs
    ):
        """
        ...
        for t in timesteps:
            ...
            if self.leak_rate is None:
                synap_input = np.dot(self._state[t - 1, :], self.w) + np.dot(
                    ext_input[t - 1, :], w_in
                )
                ...
                self._state[t, :] = (1.0 - self.leak_rate) * self._state[
                    t - 1, :
                ] + self.leak_rate * self.activation_function(synap_input, **kwargs)
...
class SpikingNeuralNetwork(Reservoir):
    ...
    def __init__(self, *args, inh=0.2, som=0.0, apply_Dale=True, **kwargs):
        """
    ...
    def simulate(
        ...
        if sig_param is not None:
            ...
            td = (1 / (1 + np.exp(-sig_param)) * (tau_max - tau_min) + tau_min) / 1000
        else:
            td = taus / 1000
        ...
        # number of spikes
        ns = 0
        tspike = np.empty((0, 2), dtype=float)
        ...
        rs = rs.reshape(rs.shape[0], int(rs.shape[1] / timescale), timescale).mean(
            axis=-1
        )
        self._state = rs.T
...
class MemristiveReservoir:
    ...
    def __init__(
        self, w, int_nodes, ext_nodes, gr_nodes, save_conductance=False, *args, **kwargs
    ):
        """
    ...
    def setW(self, w):
        ...
        if not utils.check_symmetric(w):
            ...
            W[np.triu_indices_from(w, 1)] = np.logical_or(
    ...
    def getV(self, Vi, Ve, Vgr=None):
        ...
        voltage = np.concatenate(
            [Vi[:, np.newaxis], Ve[:, np.newaxis], Vgr[:, np.newaxis]]
        ).squeeze()

        # set of all nodes (internal + external + grounded)
        nodes = np.concatenate(
            [self._I[:, np.newaxis], self._E[:, np.newaxis], self._GR[:, np.newaxis]]
        ).squeeze()
...
class MSSNetwork(MemristiveReservoir):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._Ga = np.divide(
            self.Woff, self.NMSS, where=self.NMSS != 0, out=np.zeros_like(self.Woff)
        )  # constant
        self._Gb = np.divide(
            self.Won, self.NMSS, where=self.NMSS != 0, out=np.zeros_like(self.Won)
        )  # constant
    ...
    def dG(self, V, G=None, dt=1e-4, seed=None):
        ...
        if G is not None:
            ...
            Nb = np.divide(Gdiff1, Gdiff2, where=Gdiff2 != 0, out=np.zeros_like(Gdiff1))

        else:
            Nb = self._Nb

        # ration of dt to characterictic time of the device tc
        alpha = np.divide(dt, self.tc, where=self.tc != 0, out=np.zeros_like(self.tc))
